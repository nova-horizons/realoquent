...
class MigrationWriter
{
    ...
    public function createMigrationFile(string $migrationDir, SchemaChanges $changes, string $migrationName): string
    {
        ...
        $class->addMethod('up')->setBody($this->buildFunctionBody($changes))->setReturnType('void');
    ...
    {
        $function = match (true) {
            $index->isPrimary => throw new \RuntimeException('Realoquent does not support dropping the primary key. Please do so with your own migration and re-run realoquent:generate-schema.'),
            $index->isUnique => 'dropUnique',
            default => 'dropIndex',
        };
    ...
    public function indexMigrationLine(Index $index): string
    {
        ...
        $str .= '(columns: '.var_export($index->columns, true);
