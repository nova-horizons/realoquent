...
class SchemaDiffer
{
    ...
    public function getSchemaChanges(): SchemaChanges
    {
        ...
        foreach ($this->newSchema->getTables() as $newTable) {
            ...
            foreach ($newTable->getIndexes() as $newIndex) {
                ...
                if ($oldIndex && array_diff($newIndex->getColumns(), $oldIndex->getColumns())) {
                    // TODO Can this move to Comparable?
                    $diffs['index_updated'][$newIndex->tableName.'.'.$newIndex->name]['state'] = $newIndex;
                    $diffs['index_updated'][$newIndex->tableName.'.'.$newIndex->name]['changes']['indexColumns'] = [
                        'old' => $oldIndex->getColumns(),
                        'new' => $newIndex->getColumns(),
                    ];
                }
        ...
        // TODO New columns need to support `after`
    ...
    protected function validateAndReindexCurrentSchema(Schema $schema, bool $allowEmptyIds): array
    {
        ...
        foreach ($schema->getTables() as $tableName => $table) {
            ...
            throw_if(isset($schemaByIds[$tableId]) && $tableId, new \RuntimeException('Duplicate realoquentId '.$tableId));
            ...
            foreach ($table->getColumns() as $columnName => $column) {
                ...
                throw_if(isset($schemaByIds[$columnId]) && $columnId, new \RuntimeException('Duplicate realoquentId '.$columnId));
            ...
            foreach ($table->getIndexes() as $indexName => $index) {
                ...
                throw_if(isset($schemaByIds[$indexId]) && $indexId, new \RuntimeException('Duplicate realoquentId '.$indexId));
